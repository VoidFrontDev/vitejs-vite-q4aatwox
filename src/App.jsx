...
const useGameStore = create((set, get) => ({
  ...
  updateStat: (id, stat, diff) =>
    set((s) => ({
      players: s.players.map((p) => {
        if (p.id !== id) return p;
        const newVal = Math.max(0, (p[stat] || 0) + diff);
        if (stat === 'life' && newVal <= 0 && !p.isDead)
          return { ...p, life: 0, isDead: true, deathReason: 'manual' };
        if (stat === 'poison' && newVal >= 10 && !p.isDead)
          return { ...p, [stat]: newVal, isDead: true, deathReason: 'poison' };
        return { ...p, [stat]: newVal };
      }),
    })),

  applyCmdDamage: (attackerId, targetId, diff) =>
    set((s) => {
      ...
      return {
        players: s.players.map((p) => {
          if (p.id === targetId) {
            ...
            return {
              ...p,
              life: Math.max(0, p.life - diff),
              isDead: isLethal ? true : p.isDead,
              deathReason: lethalByCmd
                ? 'commander'
                : lethalByLife
                ? 'manual'
                : p.deathReason,
          ...
          if (p.id === attackerId)
            return {
              ...p,
              damageDealt: { ...p.damageDealt, [targetId]: currentDmg + diff },
            };
          return p;
        }),
      };
  ...
  setMonarch: (id) =>
    set((s) => ({ monarchId: s.monarchId === id ? null : id })),
  toggleMonarchFeature: () =>
    set((s) => ({ monarchEnabled: !s.monarchEnabled, monarchId: null })),
  ...
  rollStartPlayer: () => {
    ...
    const interval = setInterval(() => {
      ...
      if (count > maxRounds) {
        ...
        setTimeout(
          () => set({ highlightedId: null, startingPlayerId: null }),
          3000
        );
      }
    }, 150);
  ...
  killPlayer: (id) =>
    set((s) => ({
      players: s.players.map((p) =>
        p.id === id ? { ...p, life: 0, isDead: true, deathReason: 'manual' } : p
      ),
    })),
  revivePlayer: (id) =>
    set((s) => ({
      players: s.players.map((p) => {
        if (p.id === id)
          return {
            ...
            life: 1,
            deathReason: 'manual',
            poison: p.deathReason === 'poison' ? 9 : p.poison,
          };
        return { ...p, damageDealt: { ...p.damageDealt, [id]: 0 } };
      }),
    })),
  toggleCounterType: (type) =>
    set((s) => ({
      activeCounters: { ...s.activeCounters, [type]: !s.activeCounters[type] },
    })),
  exitGame: () => set({ players: [], gameStarted: false }),
}));

// --- MOBILE OPTIMIZED LIFE ZONE ---
const LifeZone = ({ onClick, onLongPress, side }) => {
  const timerRef = useRef(null);
  const repeatRef = useRef(null);
  const isLongPress = useRef(false);

  const start = (e) => {
    // Prevent default to stop scrolling/zooming while tapping life
...
const PlayerPanel = ({
  ...
  const { updateStat, applyCmdDamage, killPlayer, revivePlayer, setMonarch } =
    useGameStore();
  ...
  const isTopRow =
    totalPlayers > 1 && (totalPlayers === 2 ? index === 0 : index < 2);
  ...
  return (
    ...
    >
        ...
        {allPlayers
          ...
          .map((target) => (
            <div key={target.id} style={cmdRowStyle}>
              ...
              >
                {player.damageDealt[target.id] || 0}
              </span>
              <div style={{ display: 'flex', gap: '6px' }}>
                <button
                  onClick={() => applyCmdDamage(player.id, target.id, -1)}
                  style={cmdBtn}
                >
                  -
                </button>
      ...
      {!player.isDead && (
        <button onClick={() => setShowCmd(!showCmd)} style={cmdToggleStyle}>
      ...
      {monarchEnabled && !player.isDead && (
        <button
          onClick={() => setMonarch(player.id)}
          style={{
            ...monarchBtnStyle,
            background: isMonarch ? '#ffd700' : 'rgba(0,0,0,0.4)',
          }}
        >
          <div
            style={{
          ...
          >
            ...
            <Crown
              size={20}
              color={isMonarch ? '#000' : '#ffd700'}
              fill={isMonarch ? '#000' : 'none'}
              style={{ position: 'absolute' }}
            />
          </div>
        </button>
      ...
      {!player.isDead && (
        <>
          <LifeZone
            ...
            onLongPress={() => updateStat(player.id, 'life', -10)}
          />
          <LifeZone
            side="right"
            onClick={() => updateStat(player.id, 'life', 1)}
        ...
        <AnimatePresence mode="wait">
          {player.isDead ? (
            ...
            >
              ...
              >
                {player.deathReason === 'commander' ? (
                  <Swords size={70} color="#ff4d4d" />
                ) : player.deathReason === 'poison' ? (
                  <Biohazard size={70} color="#4ade80" />
                ) : (
                  <Ghost size={70} color="#666" />
                )}
                <button
                  onClick={() => revivePlayer(player.id)}
                  style={reviveBtnStyle}
                >
                  <RefreshCw size={16} /> REVIVE
                </button>
              </div>
            </motion.div>
          ) : (
      ...
      {!player.isDead && (
        <div style={countersBarStyle}>
          {activeCounters.poison && (
            <CounterButton
              icon={<Skull size={14} color="#22c55e" />}
              value={player.poison}
              onClick={(d) => updateStat(player.id, 'poison', d)}
            />
          )}
          {activeCounters.rad && (
            <CounterButton
              icon={<Biohazard size={14} color="#bef264" />}
              value={player.rad}
              onClick={(d) => updateStat(player.id, 'rad', d)}
            />
          )}
          {activeCounters.energy && (
            <CounterButton
              icon={<Zap size={14} color="#3b82f6" />}
              value={player.energy}
              onClick={(d) => updateStat(player.id, 'energy', d)}
            />
          )}
          {activeCounters.exp && (
            <CounterButton
              ...
              value={player.exp}
              onClick={(d) => updateStat(player.id, 'exp', d)}
            />
          )}
        </div>
      )}
      ...
      {activeCounters.tax && !player.isDead && (
        <div style={{ ...taxWrapper, zIndex: 60 }}>
          <div style={taxOrbStyle}>
            <button
              onClick={() => updateStat(player.id, 'tax', -2)}
              style={taxBtn}
            >
              -
            </button>
            ...
            <span
              style={{ fontWeight: '900', color: '#fff', fontSize: '18px' }}
            >
              {player.tax}
            </span>
            <button
    ...
    <div
      style={{
        ...
        gridTemplateRows:
          players.length === 2
            ? '1fr 1fr'
            : `repeat(${Math.ceil(players.length / 2)}, 1fr)`,
        touchAction: 'none',
        overflow: 'hidden',
      }}
    >
      {players.map((p, i) => (
        <div
          key={p.id}
          style={{
            gridColumn: players.length === 3 && i === 2 ? '1 / span 2' : 'auto',
          }}
        >
          <PlayerPanel
            player={p}
      ...
      >
        {menuOpen && (
          ...
          >
            ...
            <button
              onClick={toggleMonarchFeature}
              style={{
                width: '100%',
                marginBottom: '12px',
                padding: '12px',
                background: monarchEnabled ? '#ffd700' : '#222',
                color: monarchEnabled ? '#000' : '#fff',
                border: 'none',
                borderRadius: '12px',
                fontWeight: 'bold',
                display: 'flex',
            ...
            >
              ...
              >
                <Crown
                  ...
                  style={{ position: 'absolute' }}
                />
                <Crown size={18} style={{ position: 'absolute' }} />
              </div>
              MONARCH: {monarchEnabled ? 'ON' : 'OFF'}
            </button>
            <div
              style={{
                display: 'grid',
            ...
            >
              {Object.keys(activeCounters).map((k) => (
                <button
                  key={k}
                  onClick={() => toggleCounterType(k)}
                  style={{
                    padding: '10px',
                    background: activeCounters[k] ? '#fff' : '#222',
                    color: activeCounters[k] ? '#000' : '#fff',
                    border: 'none',
                    borderRadius: '10px',
                    fontSize: '11px',
                    fontWeight: 'bold',
